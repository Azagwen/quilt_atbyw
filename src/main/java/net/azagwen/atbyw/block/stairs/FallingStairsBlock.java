...
import net.azagwen.atbyw.mixin.accessor.StairsBlockAccessor;
...
public class FallingStairsBlock extends AtbywStairsBlock implements LandingBlock {
    ...
    public void scheduledTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        if (canFallThrough(world.getBlockState(pos.down())) && pos.getY() >= world.getBottomY()) {
            var fallingBlockEntity = FallingBlockEntity.fall(world, pos, state);
    ...
    public void onLanding(World world, BlockPos pos, BlockState fallingBlockState, BlockState currentStateInPos, FallingBlockEntity fallingBlockEntity) {
        if (world.getBlockState(pos.down()).isOf(this) && world.getBlockState(pos.down()).get(HALF) == BlockHalf.BOTTOM) {
            ...
            world.setBlockState(pos, this.getDefaultState()
                    .with(SHAPE, StairsBlockAccessor.invokeGetStairShape(fallingBlockState, world, pos))
