...
public class RedstoneLanternBlock extends LanternBlock {
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (!player.getAbilities().allowModifyWorld) {
            ...
            world.playSound(null, pos, SoundEventRegistry.REDSTONE_LANTERN_SHRINK, SoundCategory.BLOCKS, 0.4F, (state.get(POWER_INTENSITY) / 7.5F) + 0.5F);
    ...
    public void neighborUpdate(BlockState state, World world, BlockPos pos, Block block, BlockPos fromPos, boolean notify) {
        if (state.get(LIT) == this.shouldUnpower(world, pos, state) && !world.getBlockTickScheduler().willTick(pos, this)) {
